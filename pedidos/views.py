...
def pdf_pedidos(request, id):
    ...
    html = render_to_string('pedidos/pedido/pdf.html',{'order': order, 'productos': productos})
...
def pagoCompletado(request):
    ...
    Pedido.objects.create(nombre=request.user,productos=body['Productos'],pedidoID=body['Data']['orderID'],total=body['Total'])
    cart.clear()
